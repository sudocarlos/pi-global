...
1. **Collect content** - check the existing scheme with `git tag --sort=-v:refname | head -5` (semver, calver, etc.), then generate notes via `gh release create --notes "" --dry-run` or build them from `git log --oneline <prev_tag>..HEAD`. Follow `release-notes-template.md` (in this skill directory) for section structure and formatting.
